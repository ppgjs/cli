...
import { logError, logSuccess } from '../../../shared';
...
//将整个目录压缩
export const prepareDirFileZip = (filePath: string, fileName: string): Promise<boolean> => {
...
export const prepareAliZipFile = (filePath: string): Promise<boolean> => {
  const outPutRoot = path.join(filePath, '..');
  const file = path.resolve(filePath);
  if (!existsSync(file)) {
    logError(`${file}不存在`);
  ...
  return new Promise<boolean>((resolve, reject) => {
    ...
    checkOutput.on('error', (err) => {
      logError(` checkOutput error-> ${JSON.stringify(err)}`);
      reject(err);
    });
